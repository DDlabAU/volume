...
enum interface_states { //default state should be "inactive"
	inactive,
//	volume_down,
//	volume_up,
	toggle_power
...
if (allowStateChange)
{
  if(ping<80) state=toggle_power;
  else state=inactive;

	/*
  if(ping>10) state=toggle_power;
  if(ping>80) state=volume_up;
  if(ping>120) state=volume_down;
  if(ping>160) state=inactive;
  */
}
/*
...
switch(state)
{
	...
	*/
	...
	case inactive:
		sluggishnessArmed=true;
		//volumeServo.write(volumeServoZero);
		if(stereoIsOn) setcolour(0,255,0); //white
